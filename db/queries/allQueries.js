...
const postMessage = function(
  ...
  return pool
    ...
    .then(res => {
      ...
      pool.query({
        text: `INSERT INTO user_posts(sender_id, receiver_id, message_title,sent_message)
        VALUES($1, $2, $3, $4)`,
        values: [
          data[senderUsername],
...
// const getMessage = function(receiver) {
// 	return pool
// 		.query({
// 			text: `SELECT sender_id ,receiver_id, message_title, sent_message, time_of_post FROM user_posts JOIN users ON receiver_id = users.id OR sender_id = users.id WHERE users.username = $1 ORDER BY time_of_post DESC`,
// 			values: [receiver],
...
// const getMessage = function(receiverName) {
//   return pool
//     .query({
...
//       return pool.query({
//         text: `SELECT * FROM user_posts WHERE receiver_id = $1`,
...
//     return pool
//       .query({
//         text: `SELECT username, avatar FROM users JOIN friendship ON friendship.user_id = users.id WHERE request_status = TRUE AND (user_id = $1 OR receiver_id = $1)`,
//         values: [res]
//       })
//       .then(res => res.rows);
//   });
  ...
  return pool
    ...
    .then(res3 => {
      ...
      if (res3.rows && res3.rows.length >= 1) {
        ...
        for (let i = 0; i < res3.rows.length; i++) {
          str += `$${i + 1},`;
          output.push(res3.rows[i].receiver_id);
        }
